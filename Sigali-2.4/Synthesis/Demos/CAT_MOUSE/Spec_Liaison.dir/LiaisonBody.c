...
JNIEXPORT void JNICALL Java_LiaisonBody_connexionBody
    ...
    mid_dca1=env->GetMethodID(cls,"Draw_Cat","([I)V");
    if (mid_dca1 == 0) {cout << "GetMethodID a echoue avec Draw_Cat \n";};

    mid_dca2=env->GetMethodID(cls,"Draw_Mouse","([I)V");
    if (mid_dca2 == 0) {cout << "GetMethodID a echoue avec Draw_Mouse \n";};

    mid_dca3=env->GetMethodID(cls,"Draw_Door_Cat","([I)V");
    if (mid_dca3 == 0) {cout << "GetMethodID a echoue avec Draw_Door_Cat \n";};

    mid_dca4=env->GetMethodID(cls,"Draw_Door_Mouse","([I)V");
    if (mid_dca4 == 0) {cout << "GetMethodID a echoue avec Draw_Door_Mouse \n";};
    ...
    mid_ac=env->GetMethodID(cls,"Draw_Conflict","(I)V");
    if (mid_ac == 0) {cout << "GetMethodID a echoue avec Draw_Conflict \n";};
}
...
{
  const int taille = 6; /* taille du tableau Pos_Chat */
  jint tampon[taille];
  for (int i=0; i<taille; i++) tampon[i]=(jint)Pos_Cat[i];
...
extern void Draw_Mouse(int Pos_Mouse[]){
  const int taille = 6; /* taille du tableau Pos_Souris */
  jint tampon[taille];
  for (int i=0; i<taille; i++) tampon[i]=(jint)Pos_Mouse[i]; 
  jintArray pos = env_appel_vt->NewIntArray(taille);
  env_appel_vt->SetIntArrayRegion(pos,0,taille,&*tampon);
...
extern void Draw_Door_Cat(int Door_Cat[]){
  const int taille = 7; /* taille du tableau Pos_Souris */
  jint tampon[taille];
  for (int i=0; i<taille; i++) tampon[i]=(jint)Door_Cat[i]; 
  jintArray pos = env_appel_vt->NewIntArray(taille);
  env_appel_vt->SetIntArrayRegion(pos,0,taille,&*tampon);
...
extern void Draw_Door_Mouse(int Door_Mouse[])
{
  const int taille = 7; /* taille du tableau Pos_Souris */
  jint tampon[taille];
  for (int i=0; i<taille; i++) tampon[i]=(jint)Door_Mouse[i]; 
  jintArray pos = env_appel_vt->NewIntArray(taille);
  env_appel_vt->SetIntArrayRegion(pos,0,taille,&*tampon);
			   ...
			   int cod_y[], boolean *fin_resolver)
{
  ...
  // jint tampon_cod_cond[nb_cond];
  jint *tampon_cod_cond;
  //  tampon_cod_cond  = (int *)malloc (sizeof(int)*nb_cond);
  ...
  //  jint tampon_cod_x[nb_etat];
  jint *tampon_cod_x;
  //  tampon_cod_x  = (int *)malloc (sizeof(int)*nb_etat);
  ...
  //  jint tampon_cod_u[nb_ent_cont];
  jint *tampon_cod_u;
  //  tampon_cod_u  = (int *)malloc (sizeof(int)*nb_ent_cont);
  ...
  //  jint tampon_cod_y[nb_ent_incont];
  jint *tampon_cod_y;
  //  tampon_cod_y  = (int *)malloc (sizeof(int)*nb_ent_incont);
  ...
  env_appel_vt->CallVoidMethod(
			       obj_appel_vt,mid_resolv,Cod_cond,Cod_x,Cod_u,Cod_y,Fin_resolver);
